...
pub fn create_router(state: Arc<AppState>) -> Router {
    ...
    let protected = Router::new()
        ...
        .route("/auth/2fa/verify-setup", routing::post(auth::verify_2fa_setup))
        .route("/auth/2fa/enable", routing::post(auth::enable_2fa))
        .route("/auth/2fa/disable", routing::post(auth::disable_2fa))
        .route("/auth/2fa/backup-codes", routing::get(auth::get_backup_codes))
        .route("/auth/2fa/backup-codes/regenerate", routing::post(auth::regenerate_backup_codes))
        .route("/users", routing::get(users::list_users).post(users::create_user))
        .route("/users/:id", routing::get(users::get_user).put(users::update_user).delete(users::delete_user))
        .route("/users/:id/admin", routing::put(users::set_admin))
        .route("/users/:id/disable", routing::put(users::set_disabled))
        .route("/chats", routing::get(messaging::get_chats).post(messaging::create_chat))
        .route("/chats/:id", routing::get(messaging::get_chat))
        .route("/chats/:chat_id/messages", routing::get(messaging::get_messages))
        ...
        .route("/chats/:chat_id/pins", routing::get(messaging::get_pinned_messages))
        ...
        .route("/threads/:id/messages", routing::get(messaging::get_thread_messages))
        .route("/topics", routing::get(messaging::get_topics).post(messaging::create_topic))
        .route("/typing", routing::post(messaging::send_typing))
        .route("/chats/:chat_id/read", routing::post(messaging::mark_chat_read))
        .route("/unread", routing::get(messaging::get_unread_counts))
        .route("/files/upload", routing::post(files::upload_file))
        .route("/files/upload-chat/:chat_id", routing::post(files::upload_file_for_chat))
        ...
        .route("/calls/ice/:call_id", routing::post(signaling::send_ice_candidate))
        ...
        .route("/admin/settings", routing::get(admin::get_settings).put(admin::update_settings))
        .route("/admin/settings/:key", routing::put(admin::update_setting_key))
        // Bot management (JWT protected)
        .route("/bots", routing::post(bots::create_bot).get(bots::list_bots))
        .route("/bots/:id", routing::get(bots::get_bot).put(bots::update_bot).delete(bots::delete_bot))
        .route("/bots/:id/token", routing::post(bots::regenerate_token))
        .route("/bots/:id/webhook", routing::put(bots::set_webhook).delete(bots::delete_webhook))
        .route("/bots/:id/commands", routing::get(bots::list_commands).post(bots::register_command))
