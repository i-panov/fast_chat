...
use crate::models::{MutedChat, NotificationSettings, PushSubscription};
use crate::AppState;
...
impl VapidSigner {
    ...
    fn sign(&mut self, audience: &str, subject: &str) -> Result<String, String> {
        ...
        let header = base64::engine::general_purpose::URL_SAFE_NO_PAD
            .encode(r#"{"typ":"JWT","alg":"ES256"}"#.as_bytes());
...
async fn send_to_subscription(
    ...
    let payload =
        serde_json::to_string(notification).map_err(|e| format!("Serialization error: {}", e))?;
    ...
    let resp = client
        ...
        .header(
            "Authorization",
            format!("vapid t={},k={}", vapid_sig, pubkey_b64),
        )
...
pub async fn send_push_notification(
    ...
    for sub in &subscriptions {
        ...
        if let Err(e) = result {
            tracing::warn!(
                "Failed to send push to sub {} for user {}: {}",
                sub.id,
                user_id,
                e
            );
...
pub async fn is_chat_muted(
    state: &AppState,
    user_id: Uuid,
    chat_id: Option<Uuid>,
    channel_id: Option<Uuid>,
) -> Result<bool, AppError> {
    let mute_all: Option<bool> =
        sqlx::query_scalar("SELECT mute_all FROM notification_settings WHERE user_id = $1")
            .bind(user_id)
            .fetch_optional(state.db.get_pool())
            .await?;

    if mute_all.unwrap_or(false) {
        return Ok(true);
    }
...
pub async fn unsubscribe(
    ...
    let sub_id: Uuid = id
        .parse()
        .map_err(|_| AppError::Validation("Invalid subscription ID".to_string()))?;
...
pub async fn update_settings(
    ...
    if let Some(val) = req.mute_all {
        sqlx::query(
            "UPDATE notification_settings SET mute_all = $1, updated_at = $2 WHERE user_id = $3",
        )
        .bind(val)
        .bind(now)
        .bind(user_id)
...
pub async fn mute_chat(
    ...
    if chat_id.is_none() && channel_id.is_none() {
        return Err(AppError::Validation(
            "Either chat_id or channel_id is required".to_string(),
        ));
    }
...
pub async fn test_push(
    ...
    .await?;
    Ok(Json(
        serde_json::json!({"success": true, "message": "Test push sent"}),
    ))
}
